...
public class ParentClassAdapter extends RecyclerView.Adapter<ParentClassAdapter.ChildrenClassHolder>{
    List<ModelString> classlist;
    Context context;

    public ParentClassAdapter(List<ModelString> classlist, Context context) {
    ...
    @NonNull
    @Override
    public ParentClassAdapter.ChildrenClassHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View view = inflater.inflate(R.layout.main_list_classroom,parent,false);
        return new ParentClassAdapter.ChildrenClassHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ParentClassAdapter.ChildrenClassHolder holder, int position) {
...
//        holder.image.setImageResource(Integer.parseInt(modelStringsa.getData4()));
        ...
        Glide.with(context)
                .load("http://" + ConfigData.IP + ":7777/"+modelStringsa.getData5())
